...
#let resume(
  ...
  // Level 1 Heading
  [= #author   #text(size: 14pt, weight: "regular", style: "italic")[#position]]
...
#let project(
  ...
  generic-one-by-two(
    left: {
      if role == "" {
        [*#name* #if url != "" and dates != "" [ (#link("https://" + url)[Link])]] } else {
        [*#role*, #name #if url != "" and dates != ""  [(#link("https://" + url)[Link])]]
      }
    },
    right: {
      if dates == "" and url != "" {
...
#let skills(
  ...
  for (category, skills) in categories.pairs() {
    if skills.len() > 0 {
      items.push([#strong(category): #skills.join(", ")])
    }
  }
  items.join("; ")
}
